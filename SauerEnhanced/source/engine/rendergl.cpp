...
void transplayer()
{
    ...
    //glTranslated(-player1->o.x, (player1->state==CS_DEAD ? player1->eyeheight-0.2f : 0)-player1->o.z, -player1->o.y); //from Cube ^_^
...
void gl_drawframe(int w, int h)
{
    ...
    rendermapmodels();
    rendergame(true);
    renderparticles(true, true);    //Render glow particles only
    if(!isthirdperson())
    {
        project(curavatarfov, aspect, farplane, false, false, false, avatardepth);
        game::renderavatar();
    ...
    renderparticles(true);  //Render all particles other than glow
...
void drawdamagescreen(int w, int h)
{
    ...
    static Texture *damageflashtex = NULL;
    if(!damagetex) damagetex = textureload("packages/hud/damage.png", 3);
    if(!damageflashtex) damageflashtex = textureload("packages/hud/damage2.png", 3);

    glBlendFunc(GL_ONE, GL_ONE_MINUS_SRC_ALPHA);
    glBindTexture(GL_TEXTURE_2D, game::hudplayer()->health>75?damageflashtex->id:damagetex->id);
...
{
    if(!jump)return;
    //if(oldpitch == game::hudplayer()->pitch || !oldpitch) oldpitch = game::hudplayer()->pitch;
    oldpitch = game::hudplayer()->pitch;
    if(game::hudplayer()->pitch==90)return;
    if(dojump)
    {
        dojump = false;
        game::hudplayer()->pitch = oldpitch;
    }
    dojump = true;
    inittime=lastmillis;
}

void screenjumpcheck()
{
    if(dojump && oldpitch == game::hudplayer()->pitch)
    {
        game::hudplayer()->pitch += game::hudplayer()->screenjumpheight; //20
        jmillis = lastmillis;
    }
    if(lastmillis >= (jmillis+10) && dojump && ((lastmillis-inittime) < (game::hudplayer()->screenjumpheight*10))) //milliseconds that screen will attempt to return to oldpitch
    {
        game::hudplayer()->pitch -= 2; //2 //shotgun jump takes 80 millis, magnum 180
        jmillis = lastmillis;
    }
    if(dojump && game::hudplayer()->pitch <= oldpitch)
    {
        game::hudplayer()->pitch = oldpitch;
        dojump = false;
        oldpitch = 0;
...
void gl_drawhud(int w, int h)
{
    ...
    int conw = int(w/conscale), conh = int(h/conscale), abovehud = h*2.3f - FONTH, limitgui = h*3 - FONTH;

    int textoff = 1;
    if(minimapon) textoff = 12;
    else textoff = 1;
    ...
    if(mainmenu)
    {
        ...
        int tw, th;
        defformatstring(full_se_version)("Adrenaline Cube 2 %s", se_version);
