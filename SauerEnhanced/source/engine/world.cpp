...
void renderworldaxes()
{
    ...
    glColor3f(1, 0, 0);
    glBegin(GL_LINES);
    glVertex3fv(vec(worldsize/2, worldsize/2, worldsize/2).v);
    glVertex3fv(vec( worldsize,  worldsize/2, worldsize/2).v);
    glEnd();
    ...
    glBegin(GL_LINES);
    glVertex3fv(vec(worldsize/2, worldsize/2, worldsize/2).v);
    glVertex3fv(vec(worldsize/2,   worldsize, worldsize/2).v);
    glEnd();
    ...
    glBegin(GL_LINES);
    glVertex3fv(vec(worldsize/2, worldsize/2, worldsize/2).v);
    glVertex3fv(vec(worldsize/2, worldsize/2,   worldsize).v);
    glEnd();
...
void newentity(int type, int a1, int a2, int a3, int a4, int a5, bool custom = false, vec o = vec(0, 0, 0))
{
...
#define ent entities::getents()[entgroup[i]]

void stackent(int num, int spacx, int spacy, int spacz)
{
    if(!haveselent())
    {
...
ICOMMAND(stackent, "iiii", (int *num, int *x, int *y, int *z), stackent(*num, *x, *y, *z));
