...
void modifyvelocity(physent *pl, bool local, bool water, bool floating, int curtime)
{
    ...
    if(pl->type==ENT_PLAYER)
    {
        ...
        if(pl==game::player1){
            if(abs(game::player1->lastyaw-game::player1->yaw)>3 && abs(game::player1->lastyaw-game::player1->yaw)<12 && pl->move && pl->strafe)d.mul(abs(game::player1->lastyaw-game::player1->yaw)/2.1); //13
...
//        }
    ...
    if(pl->physstate!=PHYS_FALL || game::player1->vel.magnitude2()<60 || (pl==game::player1 && abs(game::player1->lastyaw-game::player1->yaw)>0 && !pl->move && abs(game::player1->lastyaw-game::player1->yaw)<3 && pl->strafe && game::player1->vel.magnitude2()>=50))pl->vel.lerp(d, pl->vel, pow(1 - 1/fric, curtime/20.0f)); //to make no fric, disable this when player is in air
...
ICOMMAND(altattack, "D", (int *down), { game::doaltattack(*down!=0); });
ICOMMAND(sprint, "D", (int *down), { game::dosprint(*down!=0); });
ICOMMAND(crouch, "D", (int *down), { game::docrouch(*down!=0); });
