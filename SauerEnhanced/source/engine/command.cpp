...
void writeinfile(char *file, char *stuff, int *append = 0)   //easier, shorter way to write text in file
{
    stream *f = openfile(file, *append ? "a" : "w");

    if(!f)
    {
        conoutf("Error while opening/creating %s", file);
        return;
    }
