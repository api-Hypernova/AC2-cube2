...
#endif
{
    ...
    void replenishammo()
    {
        ...
        {
            baseinfo &b = bases[i];
            if(b.ammotype>0 && b.ammotype<=I_GRENADE-I_ELECTRO+1 && insidebase(b, player1->feetpos()) && player1->hasmaxammo(b.ammotype-1+I_ELECTRO)) return;
        }
    ...
    {
        type += I_ELECTRO-1;
        if(type<I_ELECTRO || type>I_GRENADE) return;
        entities::repammo(d, type, d==player1);
        int icon = itemstats[type-I_ELECTRO].icon;
    ...
    void checkitems(fpsent *d)
    {
        ...
        {
            baseinfo &b = bases[i];
            if(b.ammotype>0 && b.ammotype<=I_GRENADE-I_ELECTRO+1 && insidebase(b, d->feetpos()) && !strcmp(b.owner, d->team) && b.o.dist(o) < 12)
            {
                if(d->lastrepammo!=i)
                {
                    if(b.ammo > 0 && !player1->hasmaxammo(b.ammotype-1+I_ELECTRO)) addmsg(N_REPAMMO, "rc", d);
                    d->lastrepammo = i;
                }
                return;
            }
...
//            particle_fireball(b.ammopos, 4.8f, PART_EXPLOSION, 0, b.owner[0] ? (strcmp(b.owner, player1->team) ? 0x802020 : 0x2020FF) : 0x208020, 4.8f);
            ...
            if(b.ammotype>0 && b.ammotype<=I_GRENADE-I_ELECTRO+1)
            {
                const char *ammoname = entities::entmdlname(I_ELECTRO+b.ammotype-1);
                if(m_regencapture)
                {
                    vec height(0, 0, 0);
                ...
                else loopj(b.ammo)
                {
                    ...
                    ammopos.z += 4;
                    rendermodel(&b.light, entities::entmdlname(I_ELECTRO+b.ammotype-1), ANIM_MAPMODEL|ANIM_LOOP, ammopos, 0, 0, MDL_SHADOW | MDL_CULL_VFC | MDL_CULL_OCCLUDED);
                }
            }
    ...
    void updatebase(int i, const char *owner, const char *enemy, int converted, int ammo)
    {
        ...
        if(ammo>b.ammo)
        {
            ...
            int icon = b.ammotype>0 && b.ammotype<=I_GRENADE-I_ELECTRO+1 ? itemstats[b.ammotype-1].icon : -1;
	...
	void aifind(fpsent *d, ai::aistate &b, vector<ai::interest> &interests)
	{
		...
		loopvj(bases)
		{
			...
			if(m_regencapture)
			{
                int gun = f.ammotype-1+I_ELECTRO;
                if(f.ammo > 0 && f.ammotype > 0 && f.ammotype <= I_GRENADE-I_ELECTRO+1 && !d->hasmaxammo(gun))
    ...
    void replenishammo(clientinfo *ci)
    {
        ...
        {
            baseinfo &b = bases[i];
            if(b.ammotype>0 && b.ammotype<=I_GRENADE-I_ELECTRO+1 && insidebase(b, ci->state.o) && !ci->state.hasmaxammo(b.ammotype-1+I_ELECTRO) && b.takeammo(ci->team))
            {
                sendbaseinfo(i);
    ...
    void regenowners(baseinfo &b, int ticks)
    {
        loopv(clients)
        {
            ...
            if(ci->state.state==CS_ALIVE && ci->team[0] && !strcmp(ci->team, b.owner) && insidebase(b, ci->state.o))
            {
                ...
                if(ci->state.armour < itemstats[I_GREENARMOUR-I_ELECTRO].max)
                {
                    ci->state.armour = min(ci->state.armour + ticks*REGENARMOUR, itemstats[I_GREENARMOUR-I_ELECTRO].max);
                    notify = true;
                }
                if(b.ammotype>0)
                {
                    int ammotype = b.ammotype-1+I_ELECTRO;
                    if(ammotype<=I_GRENADE && !ci->state.hasmaxammo(ammotype))
                    {
    ...
    void initclient(clientinfo *ci, packetbuf &p, bool connecting)
    {
        ...
        {
            baseinfo &b = bases[i];
            putint(p, min(max(b.ammotype, 1), I_GRENADE+1));
