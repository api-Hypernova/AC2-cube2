...
//            }
        ...
        case N_GAMESPEED:
        {
            ...
            case N_SHOTFX:
            {
                ...
                fpsent *s = getclient(scn);
                //int holdingitem = getint(p);
                if(!s) break;
                //if(gun>GUN_FIST && gun<=GUN_PISTOL && s->ammo[gun]) s->ammo[gun]-=guns[s->gunselect].sub;
                s->ammo[s->gunselect]-=guns[s->gunselect].sub;
                if(s->gunselect==GUN_SG)s->ammo[GUN_SHOTGUN2]-=guns[GUN_SG].sub;
                if(s->gunselect==GUN_SHOTGUN2)s->ammo[GUN_SG]-=guns[GUN_SHOTGUN2].sub;
                ...
                //if(holdingitem==1)s->isholdingnade=1;
                //else if(holdingitem==2)s->isholdingorb=1;
            ...
            {
                if(!d) return;
                //conoutf(CON_GAMEINFO, "Received catch packet");
                int isholdingnade=getint(p);
...
//        }
            ...
            case N_ITEMSPAWN:
            {
                ...
                ai::itemspawned(i);
                if(entities::ents[i]->type!=I_GREENARMOUR && entities::ents[i]->type!=I_BOOST && entities::ents[i]->type!=I_YELLOWARMOUR)playsound(S_ITEMSPAWN, &entities::ents[i]->o, NULL, 0, 0, -1, 0, 1500);
                if(sespawnfx)
                {
                    if(entities::ents[i]->type!=I_GREENARMOUR && entities::ents[i]->type!=I_BOOST && entities::ents[i]->type!=I_YELLOWARMOUR)particle_fireball(entities::ents[i]->o, 10.1f, PART_EXPLOSION_BLUE, 250, 0xFFFF00, 10.1f);
            ...
            {
                int t = getint(p);
                if     (t==I_QUAD)  { playsound(S_V_QUAD10, NULL, NULL, 0, 0, -1, 0, 3000);  conoutf(CON_GAMEINFO, "\f2Damage Amplifier will spawn in 10 seconds!"); }
