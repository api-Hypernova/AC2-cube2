...
struct gamestate : fpsstate
{
    ...
    int lastshot;
    projectilestate<24> rockets, orbs, smgnades, xbolts, electro, grenades, props, barrels;
...
bool canspawnitem(int type) { return !m_noitems && (type>=I_ELECTRO && type<=I_QUAD && (!m_noammo || type<I_ELECTRO || type>I_GRENADE)); }
...
int spawntime(int type)
{
    ...
    case I_SMALLHEALTH:
    case I_SHOTGUN: msec = np*6*1000; break;
...
// int scaletime(int t) { return t*gamespeed; } //ends here
...
void explodeevent::process(clientinfo *ci)
{
    ...
    case GUN_HANDGRENADE:
        ...
        break;
    case GUN_CG2: //#don't let the server remove orbs on the first impact
        //if(!gs.orbs.remove(id)) return;
        break;
    case GUN_SMG2:
        if(!gs.smgnades.remove(id)) return;
        break;
    case GUN_FIREBALL:
        if(!gs.orbs.remove(id)) return;
        break;
    case GUN_BARREL:
        if(!gs.barrels.remove(id)) return;
        break;
    case GUN_BITE:
        if(!gs.props.remove(id)) return;
        break;
        //        case GUN_ELECTRO:
    ...
    loopv(hits)
    {
        ...
        if(!target || target->state.state!=CS_ALIVE || h.lifesequence!=target->state.lifesequence || h.dist<0 || h.dist>guns[gun].splash) continue;
...
void shotevent::process(clientinfo *ci)
{
    ...
    //wait<gs.gunwait ||
    //    (gs.ammo[gs.gunselect]-guns[gs.gunselect].sub)<0 ||
    // gun<GUN_FIST || gun>GUN_PISTOL || (guns[gun].range && from.dist(to) > guns[gun].range + 1))
    //   return;
    gs.ammo[gs.gunselect]-=guns[gs.gunselect].sub;
    if(gs.gunselect==GUN_SG) gs.ammo[GUN_SHOTGUN2]-=guns[GUN_SG].sub;
    if(gs.gunselect==GUN_SHOTGUN2) gs.ammo[GUN_SG]-=guns[GUN_SHOTGUN2].sub;
    //gs.gunwait = 2; //guns[gun].attackdelay;
    ...
    gs.lastshot = millis;
    gs.shotdamage += guns[gun].damage*(int)(gs.quadmillis ? 2 : 1)*(guns[gun].rays);
    switch(gun)
    {
    case GUN_RL: gs.rockets.add(id); break;
    case GUN_CG2: gs.orbs.add(id); break;
    ...
    default:
    {
        ...
        loopv(hits)
        {
            ...
            //if(((fpsent*)target)->type==ENT_INANIMATE)conoutf(CON_GAMEINFO, "hit barrel");
            //if(hits.length()==0)conoutf(CON_GAMEINFO, "hit barrel");
            //if(headshot(((dynent *)ci), from, to)) damage *= 3;
            if(gs.quadmillis) damage *= 1.5f;
            //if(h.headshots) { damage*=3;}
            loopi(h.rays)
            {
                if(i<h.headshots)damage+=(guns[gun].damage*(maxrays>1?2:3));//add 2x more damage for that shot for a total of 3x headshot damage
            }
...
#define QUEUE_STR(text) QUEUE_BUF(sendstring(text, cm->messages))
    ...
    case N_POS:
    {
        ...
        if(cp)
        {
            ...
            {
                if(!ci->local && !m_edit && max(vel.magnitude2(), (float)fabs(vel.z)) >= INT_MAX-1) //TROLOLOL
                    cp->setexceeded();
    ...
    case N_EDITMODE:
    {
        ...
        if(val)
        {
            ...
            ci->state.barrels.reset();
            //ci->state.tkobjs.reset();
        }
...
const char *defaultmaster() { return "192.168.137.10"; }
